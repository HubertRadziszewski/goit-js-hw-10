...
function searchForCountry(name) {
    const search = (0, _debounceDefault.default)(()=>{
        (0, _fetchCountries.fetchCountries)(name).then((countryList)=>{
            console.log(countryList);
            if (countryList.length === 0) (0, _notiflixDefault.default).Notify.failure("Oops, there is no country with that name.");
            else if (countryList.length > 10) (0, _notiflixDefault.default).Notify.failure("Too many matches found. Please enter a more specific name.");
            else {
                const html = countryList.map((countryInfo)=>{
                    return `<div class="country">` + `<p class="name">` + `<img width="64" height="32" src="${countryInfo.flags.svg}" alt="${countryInfo.flags.alt}"></img>` + `<span>${countryInfo.name.common}</span>` + `</p>` + `<p class="capital">` + `<b>Capital: </b>` + `<span>${countryInfo.capital}</span>` + `</p>` + `<p class="population">` + `<b>Population: </b>` + `<span>${countryInfo.population}</span>` + `</p>` + `<p class="languages">` + `<b>Languages: </b>` + `<span>${formatLanguages(countryInfo.languages)}</span>` + `</p>` + `</div>`;
                }).join("");
                res.innerHTML = html;
            }
        }).catch((err)=>{
            if (err.message.includes("404")) (0, _notiflixDefault.default).Notify.failure("Oops, there is no country with that name.");
            else (0, _notiflixDefault.default).Notify.failure(err.message);
        });
    }, 300);
    search();
}
...
inp.addEventListener("input", ()=>{
    ...
    if (country.trim() === "") clearResults();
});

},{"notiflix":"fuMq1","debounce":"6mekx","./fetchCountries":"7aRSZ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fuMq1":[function(require,module,exports) {
var global = arguments[3];
...
},{}],"7aRSZ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{}]},["3pyWu","4pp4s"], "4pp4s", "parcelRequireabb0")
